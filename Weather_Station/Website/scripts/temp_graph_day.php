<?php
	/* Temperature Graph - Last Day */
	
	/*
	* Retrieves the temperature data for the last day to display in its chart
	*/
	
	require_once ("db_connect.php"); // Connects to database

	// Database query
	$query = "SELECT DATE_SUB(date_time, INTERVAL 1 MONTH) AS TIME, HOUR(date_time), AVG(outward_temperature), MIN(outward_temperature), MAX(outward_temperature)
